...
<main id="content">
    ...
    <section class="sc-career career-why">
        <div class="container">
            <div class="sc-inner">
                <div class="sc-heading animate fadeInUp">
                    <h2 class="sc-headline">{!! html_entity_decode(isset($data->montivory)?$data->montivory->title:'Why Montivory') !!}</h2>
                </div>
                <div class="career-content animate fadeInUp">
                    {!! html_entity_decode(isset($data->montivory)?$data->montivory->content:'<p>Today, many organizations proudly tell us that they have successfully transformed themselves digitally. But no one shares the story of what they did before they were successful. So where should
                        we start?</p>
    ...
    <section class="sc-career career-benefit sc-white">
        <div class="container">
            <div class="sc-inner">
                <div class="sc-heading animate fadeInUp">
                    <h2 class="sc-headline">{!! html_entity_decode(isset($data->benefit)?$data->benefit->title:'Our Benefit') !!}</h2>
                </div>
                <div class="career-content animate fadeInUp">
                    {!! html_entity_decode(isset($data->benefit)?$data->benefit->content:'<p class="headline"><strong>Lifestyle at Montivory</strong></p>
                ...
                @if(isset($data->benefit))
                    @if(count($data->benefit->gallery) > 0)
                        <div class="swiper-container loop multiple center benefit animate fadeInUp">
                            <div class="swiper-wrapper">
                                @foreach ($data->benefit->gallery as $gallery)
                                    <div class="swiper-slide benefit-card">
                                        <div class="image object-fit"><img alt="" src="{{ asset($gallery->url)}}"></div>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="skill-panel">
                            <ul class="skill-list" id="interest-list">
                                @foreach ($interests as $interests)
                                    <li name="{{ $interests }}">{{ $interests }}</li>
                                @endforeach
                            </ul>
                            ...
                            <div class="field">
                                <div class="input select">
                                    ...
                                    <select data-placeholder="Please select" class="select2" id="position" name="position">
                                        ...
                                        <option value="{{$position->sys->id}}">{{$position->fields->title}}</option>
                                        @endforeach
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                $.each(response.data,function(key,value){
                    let str = `
                        <div class="skill-result-box fadeIn">
                            <h3 class="skill-result-position">${value.title}</h3>
                            <div class="skill-result-description">
                                <p>${value.excerpt}</p>
                            </div>
                            <a href="#joinus" position="${value.id}" class="menu-scroll apply-job">APPLY NOW</a> <a href="{{url('/career')}}/${value.slug}" class="menu-scroll apply-job">DETAIL</a>
                        </div>
                        `;
                        $('#skill-result-block').append(str);
                        $('#position').append(`<option value="${value.id}">${value.title}</option>`).fadeIn(1000);
                });
                $('#position').append('<option value="other">Other</option>');
                let lastblock = '<div class="skill-result-remark"><p>Can\'t find the right one?</p><a href="#joinus" class="und und-blue apply-job" position="other">CONTACT US</a></div>';
