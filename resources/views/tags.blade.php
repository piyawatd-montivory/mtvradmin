    ...
    <div class="container pt-5">
        ...
        <div class="row page-row">
            <div class="col-12 text-center d-none d-md-block">
                ...
                @endphp
                <a href="{{ route('tags',['slug'=>$tags->id,'page'=>1]) }}{{$params}}" class="btn btn-page btn-page-nav @if(!$page->first) d-none @endif first-btn">FIRST</a>
                <a href="{{ route('tags',['slug'=>$tags->id,'page'=>$currentPage-1]) }}{{$params}}" class="btn btn-page btn-page-nav @if(!$page->first) d-none @endif previous-btn">PREVIOUS</a>
                @for($i = $page->min;$i <= $page->max;$i++)
                    @if($i == $currentPage)
                        <a href="javascript:void(0);" class="btn btn-page active">{{ $i }}</a>
                    @else
                        <a href="{{ route('tags',['slug'=>$tags->id,'page'=>$i]) }}{{$params}}" class="btn btn-page">{{ $i }}</a>
                    @endif
                @endfor
                <a href="{{ route('tags',['slug'=>$tags->id,'page'=>$currentPage+1]) }}{{$params}}" class="btn btn-page btn-page-nav @if(!$page->last) d-none @endif next-btn">NEXT</a>
                <a href="{{ route('tags',['slug'=>$tags->id,'page'=>$data->pages]) }}{{$params}}" class="btn btn-page btn-page-nav @if(!$page->last) d-none @endif last-btn">LAST</a>
            </div>
            <div class="col-12 text-center d-md-none">
                <a href="{{ route('tags',['slug'=>$tags->id,'page'=>1]) }}{{$params}}" class="btn btn-page btn-page-nav @if(!$page->first) d-none @endif first-btn">
                    <img src="{{ asset('images/icon/btn-first-mb.png')}}"/>
                </a>
                <a href="{{ route('tags',['slug'=>$tags->id,'page'=>$currentPage-1]) }}{{$params}}" class="btn btn-page btn-page-nav @if(!$page->first) d-none @endif previous-btn">
                ...
                @for($i = $page->min;$i <= $page->max;$i++)
                    ...
                    @endif
                @endfor
                <a href="{{ route('tags',['slug'=>$tags->id,'page'=>$currentPage+1]) }}{{$params}}" class="btn btn-page btn-page-nav next-btn @if(!$page->last) d-none @endif ">
                    <img src="{{ asset('images/icon/btn-next-mb.png')}}"/>
                </a>
                <a href="{{ route('tags',['slug'=>$tags->id,'page'=>$data->pages]) }}{{$params}}" class="btn btn-page btn-page-nav last-btn @if(!$page->last) d-none @endif ">
...
<script type="text/javascript">
    ...
    const filtercontent = (typewindows) => {
        ...
        window.location.href = "/tags/{{ $tags->id }}"+params;
    }
