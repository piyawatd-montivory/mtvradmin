        ...
        <script type="text/javascript">
            ...
            const selectedImage = () => {
                if($('.selected').length > 0)
                {
                    @if($type == 'single')
                        let imageObj = {}
                        imageObj.id = $($('.selected')[0]).attr('imageid');
                        imageObj.url = $($('.selected')[0]).find('.card-img-top').attr('src');
                        imageObj.title = $($('.selected')[0]).find('.card-title').text();
                        imageObj.description = $($('.selected')[0]).find('.imagedescription').val();
                        parent.selImageSuccess(imageObj);
                    @else
                        let imageLists = [];
                        $.each($('.selected'),function(index,value){
                            let imageObj = {}
                            imageObj.id = $(value).attr('imageid');
                            imageObj.url = $(value).find('.card-img-top').attr('src');
                            imageObj.title = $(value).find('.card-title').text();
                            imageObj.description = $(value).find('.imagedescription').val();
                            imageLists.push(imageObj);
                        })
                        parent.selImageSuccess(imageLists);
                    @endif
                }
